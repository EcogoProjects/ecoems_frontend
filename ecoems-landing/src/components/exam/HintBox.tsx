...
interface HintBoxProps {
    ...
    onClose: () => void; // Añadimos la función para cerrar el modal
}

const HintBox = ({ onShowHint, onShowExplanation, onClose }: HintBoxProps) => {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            {/* Capa de fondo con blur: al darle clic, cierra el modal */}
            <div
                className="absolute inset-0 bg-black/20 backdrop-blur-[2px]"
                onClick={onClose}
            />

            {/* Contenedor blanco principal (con position: relative para la X) */}
