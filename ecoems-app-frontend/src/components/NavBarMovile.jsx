    ...
    return (  
        <div className="fixed bottom-1 left-1/2 -translate-x-1/2 w-[92%] max-w-[420px] z-50 md:hidden">
            ...
            <div className="bg-base-dark/90 backdrop-blur-md border border-white/10 flex items-center justify-around py-3 px-4 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] text-white">
                ...
                {/* Analytics */}
                <Link href="/app/analytics" className={`p-2 transition-transform active:scale-90 hover:opacity-80 ${pathname === '/app/analytics' ? 'bg-white/10 rounded-2xl' : ''}`}>
                    <ChartBarBigColumns height="28px" width="28px" pack="filled"/>
                </Link>

                {/* Home - Icono central un poco más grande */}
                <Link href="/app/home" className={`p-3 rounded-2xl transition-all active:scale-90 shadow-inner ${pathname === '/app/home' ? 'bg-white/10' : ''}`}>
                    <FaHome size={28}/>
                </Link>

                {/* Profile */}
                <Link href="/app/profile" className={`transition-transform active:scale-90 ${pathname === '/app/profile' ? 'bg-white/10 rounded-2xl p-2' : ''}`}>
